...
class Item:
    # VK: added beam_rep and beam_hash to allow assigning each NODE its rep and hash
    def __init__(self, curr_type, l_child_idx, r_child_idx, mask, beam_rep=None, beam_hash=None):
        self.curr_type = curr_type
        self.l_child_idx = l_child_idx
        self.r_child_idx = r_child_idx
        self.mask = mask
        self.beam_rep = beam_rep
        self.beam_hash = beam_hash
...
def reconstruct_tree(
    ...
    tuple_el.representation = items[cnt].beam_rep[batch_el][idx]
    tuple_el.hash = items[cnt].beam_hash[batch_el][idx].item()
    # print(f"{tuple_el.representation.shape=}, {items[cnt].beam_rep.shape=}")  # [dim e.g. 256], [batch_size, num elems in beam, dim]
    if cnt > 0:
