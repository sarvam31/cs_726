# Source: https://github.com/OhadRubin/SmBop/blob/main/smbop/utils/ra_preproc.py
...
def ast_to_ra(ast_dict, args=None):
    ...
    if "select_distinct" in ast_dict:
        select_node = codegen_select(ast_dict["select_distinct"], args)
        distinct_node = Node("Distinct", parent=root, n_type="Table")  # Create a distinct node
        select_node.parent = distinct_node  # Attach the select node to the distinct node
        node = distinct_node  # Set the current node to the distinct node
    else:
